...
static void writeAll(int fd, const void* data, std::size_t size) {
  ...
  while (size > 0) {
    ...
    if (n <= 0) throw std::runtime_error("write() to Python worker failed");
    ptr += n;
...
static void readAll(int fd, void* data, std::size_t size) {
  ...
  while (size > 0) {
    ...
    if (n <= 0) throw std::runtime_error("read() from Python worker failed");
    ptr += n;
    size -= static_cast<std::size_t>(n);
  }
}
...
static PixelFormat strToFmt(const std::string& s) {
  if (s == "BGR8") return PixelFormat::BGR8;
  if (s == "Gray8") return PixelFormat::Gray8;
...
static std::string buildRequestJson(const IPythonRuntime::Request& req) {
  ...
  ss << "},\"w\":" << req.w << ",\"h\":" << req.h << ",\"c\":" << req.c
     << ",\"fmt\":\"" << fmtToStr(req.fmt) << "\"}";
  return ss.str();
}
...
static IPythonRuntime::Response parseResponse(const std::string& json, int fd) {
  ...
  const std::size_t imgSize =
      static_cast<std::size_t>(resp.w) * resp.h * resp.c;
...
PythonSubprocessRuntime::~PythonSubprocessRuntime() { shutdown(); }
...
void PythonSubprocessRuntime::ensureStarted() {
  ...
  if (workerPid_ == 0) {
    ...
    ::close(stdoutPipe[0]);
    ::close(stdoutPipe[1]);
    ::execl(pythonPath_.c_str(), pythonPath_.c_str(), workerPath_.c_str(),
            nullptr);
    ::_exit(127);
  }
