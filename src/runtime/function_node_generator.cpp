...
std::vector<NodeDescriptor> FunctionNodeGenerator::generate(
    const std::vector<FunctionDescriptor>& functions) const {
  ...
  for (const auto& function : functions) {
    ...
    node.displayName = function.displayName.empty() ? function.functionName
    ...
    node.factoryKey =
        function.factoryKey.empty() ? node.id : function.factoryKey;
    node.ui.title = function.library + ": " + function.functionName + "()";
    node.ui.summary = function.description;
    ...
    for (const auto& argument : function.arguments) {
      if (argument.direction == ArgumentDirection::Input ||
        ...
        node.inputs.push_back(PortDescriptor{
            .id = argument.name,
            .displayName = argument.displayName.empty() ? argument.name
      ...
      if (argument.direction == ArgumentDirection::Output ||
          argument.direction == ArgumentDirection::InputOutput) {
