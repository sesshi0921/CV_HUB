...
FunctionNodeGenerator::generate(const std::vector<FunctionDescriptor>& functions) const {
    ...
    for (const auto& function : functions) {
        ...
        node.displayName =
            function.displayName.empty() ? function.functionName : function.displayName;
        ...
        for (const auto& argument : function.arguments) {
            if (argument.direction == ArgumentDirection::Input ||
                ...
                node.inputs.push_back(PortDescriptor{
                    .id = argument.name,
                    .displayName =
                        argument.displayName.empty() ? argument.name : argument.displayName,
