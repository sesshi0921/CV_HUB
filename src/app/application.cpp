...
#ifdef CVHUB_HAS_PYTORCH_PLUGIN
...
std::shared_ptr<ServiceContainer> createApplication(
...
#ifdef CVHUB_HAS_PYTORCH_PLUGIN
  plugins::pytorch::registerPyTorchPlugin(*services, CVHUB_PYTORCH_PYTHON,
                                          CVHUB_PYTORCH_WORKER_PATH);
#endif
...
void printUsage() {
  std::cout << "Usage: cvhub [--list-nodes] [--run-sample] [--log-file <path>] "
               "[--workers <count>]\n";
}
...
int runCli(int argc, char** argv) {
  ...
  if (listNodes) {
    for (const auto& node : services->nodeCatalog()->list()) {
      std::cout << node.id << " | " << node.displayName << " | "
                << node.category << "\n";
    }
  }
  ...
  if (runSample || (!listNodes && !runSample)) {
    const auto result =
        services->pipelineExecutor()->run(createOpenCVSampleGraph());
    std::cout << result.message << "\n";
    if (!result.ok) {
      return 1;
    }
    if (const auto lastNode = result.outputs.find("blur");
        lastNode != result.outputs.end()) {
      if (const auto imageOutput = lastNode->second.find("image");
          imageOutput != lastNode->second.end()) {
        const auto image = valueCast<ImageValue>(imageOutput->second);
        if (image) {
          std::cout << "final image: " << image->width() << "x"
