...
inline IPythonRuntime::Request imageToRequest(const std::string& fn, const ImageValue& img,
    ...
    const auto rowBytes = static_cast<std::size_t>(img.width()) * img.channels();
    ...
    for (int row = 0; row < img.height(); ++row) {
        std::memcpy(req.imageBytes.data() + static_cast<std::size_t>(row) * rowBytes,
                    bytes.data() + static_cast<std::size_t>(row) * img.strideBytes(), rowBytes);
    }
    return req;
}

inline std::shared_ptr<ImageValue> responseToImage(const IPythonRuntime::Response& resp) {
    if (!resp.ok)
        throw std::runtime_error("Python worker error: " + resp.error);
    return std::make_shared<ImageValue>(resp.w, resp.h, resp.c, resp.w * resp.c, resp.fmt,
                                        resp.imageBytes);
}

inline std::shared_ptr<INodeFactory> makePythonTransformFactory(NodeDescriptor descriptor,
                                                                std::string fnName) {
    auto runtime = requirePythonRuntime();
    ImageTransformFn fn = [runtime, fnName = std::move(fnName)](
    ...
    return std::make_shared<GenericImageTransformFactory>(std::move(descriptor), std::move(fn));
}
