...
class ImageValue final : public ValueBase {
  public:
    ImageValue(int width, int height, int channels, int strideBytes, PixelFormat pixelFormat,
               std::vector<std::byte> bytes);

    SemanticType type() const override {
...
template <typename T> std::shared_ptr<const T> valueCast(const ValuePtr& value) {
