...
enum class ArgumentDirection { Input, Output, InputOutput, Parameter };
...
class IFunctionNodeGenerator {
 public:
  ...
  virtual std::vector<NodeDescriptor> generate(
      const std::vector<FunctionDescriptor>& functions) const = 0;
};

class FunctionNodeGenerator final : public IFunctionNodeGenerator {
 public:
  std::vector<NodeDescriptor> generate(
      const std::vector<FunctionDescriptor>& functions) const override;
};
