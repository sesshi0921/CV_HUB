...
class ServiceContainer {
  public:
    ...
    std::shared_ptr<INodeCatalog> nodeCatalog() const {
        return nodeCatalog_;
    }
    std::shared_ptr<IFunctionNodeGenerator> functionNodeGenerator() const {
        return functionNodeGenerator_;
    }
    std::shared_ptr<ILogger> logger() const {
        return logger_;
    }
    std::shared_ptr<IPipelineExecutor> pipelineExecutor() const {
        return pipelineExecutor_;
    }
