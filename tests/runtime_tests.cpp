...
int main() {
    try {
        auto services =
            cvhub::app::createApplication({.logFile = "cvhub-tests.log", .workerCount = 2});
        const auto nodes = services->nodeCatalog()->list();
        require(!nodes.empty(), "node catalog should not be empty");
        require(services->nodeCatalog()->descriptor("opencv.resize").id == "opencv.resize",
                "opencv.resize should be registered");

        const auto result =
            services->pipelineExecutor()->run(cvhub::app::createOpenCVSampleGraph());
