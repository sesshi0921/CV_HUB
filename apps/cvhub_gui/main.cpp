...
struct PipelineWorker {
    ...
    void start(std::shared_ptr<cvhub::IPipelineExecutor> exec, cvhub::PipelineGraph initial) {
        active = true;
        {
...
void uploadNodePreviews(const cvhub::PipelineRunResult& result,
    ...
    for (const auto& [instanceId, ports] : result.outputs) {
        for (const auto& [portId, valuePtr] : ports) {
            ...
            glPixelStorei(GL_UNPACK_ROW_LENGTH, img->strideBytes() / std::max(1, bpp));
            const auto* data = reinterpret_cast<const GLubyte*>(img->bytes().data());

            if (img->channels() == 1) {
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, img->width(), img->height(), 0, GL_RED,
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img->width(), img->height(), 0, GL_BGR,
                             GL_UNSIGNED_BYTE, data);
            } else {
...
void cleanupPreviews(std::unordered_map<std::string, NodePreviewTexture>& previews) {
...
std::optional<std::string> browseJsonPath(bool saveDialog, const std::string& currentPath) {
...
void setAppWindowIcon(GLFWwindow* window) {
    ...
    image.pixels = const_cast<unsigned char*>(cvhub::gui::resources::kAppIconRgba.data());
    glfwSetWindowIcon(window, 1, &image);
    cvhub::gui::applyNativeAppIcon();
}

void DrawFolderGlyph(const ImVec2& p, const ImVec2& q, bool active, bool hovered) {
    ImDrawList* dl = ImGui::GetWindowDrawList();
    const ImU32 bg = active    ? IM_COL32(255, 132, 42, 255)
    ...
    dl->AddRectFilled(ImVec2(x0 + 2.0f, p.y + h * 0.28f), ImVec2(x0 + w * 0.34f, y0 + 2.0f), fg,
                      2.0f);
...
bool FolderIconButton(const char* id) {
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.92f, 0.45f, 0.15f, 1.00f));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(1.00f, 0.50f, 0.18f, 1.00f));
...
bool InputPathWithFolderButton(const char* id, char* buffer, std::size_t bufferSize,
                               bool saveDialog) {
    const float actualButtonWidth = 46.0f;
    const float spacing = ImGui::GetStyle().ItemSpacing.x;
    ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x - actualButtonWidth - spacing);
    ...
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("%s JSON file", saveDialog ? "Choose a save path for the" : "Choose a");
    return changed;
}
...
bool ToggleSwitch(const char* id, bool* value) {
    ...
    if (clicked)
        *value = !*value;
    ImDrawList* dl = ImGui::GetWindowDrawList();
    const ImU32 bg = *value ? IM_COL32(38, 139, 210, 255) : IM_COL32(80, 80, 85, 255);
    dl->AddRectFilled(pos, ImVec2(pos.x + kW, pos.y + kH), bg, kR);
    const float knobX = *value ? pos.x + kW - kR : pos.x + kR;
    dl->AddCircleFilled(ImVec2(knobX, pos.y + kR), kR - 2.0f, IM_COL32(255, 255, 255, 230));
    return clicked;
}
...
cvhub::ParameterValue deserializeParam(const json& j) {
    ...
    if (t == "i")
        return j.at("v").get<int>();
    if (t == "d")
        return j.at("v").get<double>();
    if (t == "b")
        return j.at("v").get<bool>();
...
void saveGraph(const std::string& path, const GuiGraphState& state) {
    ...
    for (const auto& n : state.nodes) {
        ...
        for (const auto& [k, v] : n.node.parameters)
...
GuiGraphState createGuiGraphState(const cvhub::PipelineGraph& graph,
                                  const std::shared_ptr<cvhub::INodeCatalog>& catalog) {
    GuiGraphState state;
    state.edges = graph.edges;

    const ImVec2 positions[] = {ImVec2(40.0f, 80.0f), ImVec2(300.0f, 80.0f), ImVec2(560.0f, 80.0f),
                                ImVec2(820.0f, 80.0f)};
...
int findNodeIndex(const GuiGraphState& state, const cvhub::NodeInstanceId& instanceId) {
    const auto it = std::ranges::find_if(
        state.nodes, [&](const GuiNodeState& node) { return node.node.instanceId == instanceId; });
...
int findInputPortIndex(const cvhub::NodeDescriptor& descriptor, const cvhub::PortId& portId) {
    const auto it = std::ranges::find_if(
        descriptor.inputs, [&](const cvhub::PortDescriptor& port) { return port.id == portId; });
...
int findOutputPortIndex(const cvhub::NodeDescriptor& descriptor, const cvhub::PortId& portId) {
    const auto it = std::ranges::find_if(
        descriptor.outputs, [&](const cvhub::PortDescriptor& port) { return port.id == portId; });
    return it == descriptor.outputs.end()
               ? -1
               : static_cast<int>(std::distance(descriptor.outputs.begin(), it));
}

void renderNodeList(const std::vector<cvhub::NodeDescriptor>& nodes, GuiGraphState& graphState,
                    float panelWidth, bool autoRun,
    ...
    auto renderItem = [&](const cvhub::NodeDescriptor* nd) {
        ...
        if (ImGui::IsItemClicked()) {
            ...
            newNode.instanceId = nd->library + ":" + nd->functionName + "()-" + std::to_string(idx);
            ...
            gs.descriptor = *nd;
            gs.position = ImVec2(100.0f + static_cast<float>(idx % 5) * 50.0f, 200.0f);
            gs.running = !autoRun;
...
void renderGraph(GuiGraphState& state, float reserveRight,
    ...
    if (ImGui::IsWindowHovered(ImGuiHoveredFlags_ChildWindows |
        ...
        if (wheel != 0.0f) {
            const float newZoom = std::clamp(state.zoom * std::pow(1.12f, wheel), 0.15f, 4.0f);
            if (std::abs(newZoom - state.zoom) > 0.001f) {
                ...
                const ImVec2 center{-panning.x + winSz.x * 0.5f, -panning.y + winSz.y * 0.5f};
    ...
    for (std::size_t i = 0; i < state.nodes.size(); ++i) {
        ...
        if (isSelected) {
            ImNodes::PushColorStyle(ImNodesCol_NodeOutline, IM_COL32(255, 140, 0, 255));
        }
        ...
        if (graphNode.running) {
            ...
            ImNodes::PushColorStyle(ImNodesCol_TitleBarHovered, IM_COL32(65, 130, 200, 255));
            ImNodes::PushColorStyle(ImNodesCol_TitleBarSelected, IM_COL32(80, 150, 220, 255));
        } else {
            ImNodes::PushColorStyle(ImNodesCol_TitleBar, IM_COL32(70, 70, 75, 255));
            ImNodes::PushColorStyle(ImNodesCol_TitleBarHovered, IM_COL32(90, 90, 95, 255));
            ImNodes::PushColorStyle(ImNodesCol_TitleBarSelected, IM_COL32(110, 110, 115, 255));
        }

        constexpr float kNodeWidth = 220.0f;
        constexpr float kToggleW = 28.0f;

        const bool hasImageOutput =
            std::any_of(graphNode.descriptor.outputs.begin(), graphNode.descriptor.outputs.end(),
        ...
        if (hasImageOutput && graphNode.previewEnabled) {
            ...
            if (texIt != previews.end() && texIt->second.id != 0) {
                ...
                const float aspect = static_cast<float>(tex.height) / static_cast<float>(tex.width);
        ...
        for (std::size_t p = 0; p < portRows; ++p) {
            const float inW =
                p < inputs.size() ? ImGui::CalcTextSize(inputs[p].displayName.c_str()).x : 0.0f;
            const float outW =
                p < outputs.size() ? ImGui::CalcTextSize(outputs[p].displayName.c_str()).x : 0.0f;
        ...
        if (!graphNode.descriptor.parameters.empty()) {
            ...
            for (const auto& param : graphNode.descriptor.parameters) {
                ...
                if (it == graphNode.node.parameters.end())
                    continue;
                const std::string widgetId = "##" + param.name + graphNode.node.instanceId;
                if (!param.options.empty()) {
                    ...
                    const int minV = param.minValue ? static_cast<int>(*param.minValue) : 0;
                    const int maxV = param.maxValue ? static_cast<int>(*param.maxValue) : 100;
                    ...
                    const float minV = param.minValue ? static_cast<float>(*param.minValue) : 0.0f;
                    const float maxV = param.maxValue ? static_cast<float>(*param.maxValue) : 1.0f;
        ...
        if (hoveredNodeId == uiNodeId && ImGui::IsMouseReleased(ImGuiMouseButton_Right)) {
    ...
    for (std::size_t i = 0; i < state.edges.size(); ++i) {
        ...
        const int fromPortIndex = findOutputPortIndex(
            state.nodes[static_cast<std::size_t>(fromIndex)].descriptor, edge.fromPort);
    ...
    if (ImNodes::IsLinkCreated(&startAttr, &endAttr)) {
        ...
        if (fromNodeIndex >= 0 && fromNodeIndex < static_cast<int>(state.nodes.size()) &&
            toNodeIndex >= 0 && toNodeIndex < static_cast<int>(state.nodes.size()) &&
            fromPortIndex >= 0 && toPortIndex >= 0) {
            const auto& fromNode = state.nodes[static_cast<std::size_t>(fromNodeIndex)];
            const auto& toNode = state.nodes[static_cast<std::size_t>(toNodeIndex)];
            if (fromPortIndex < static_cast<int>(fromNode.descriptor.outputs.size()) &&
                ...
                state.edges.push_back(cvhub::GraphEdge{
                    ...
                    .fromPort =
                        fromNode.descriptor.outputs[static_cast<std::size_t>(fromPortIndex)].id,
                    .toNode = toNode.node.instanceId,
                    .toPort = toNode.descriptor.inputs[static_cast<std::size_t>(toPortIndex)].id,
                });
            }
        }
    }
...
void renderProperties(GuiGraphState& state,
    ...
    state.selectedIndex =
        std::clamp(state.selectedIndex, 0, static_cast<int>(state.nodes.size() - 1));
    ...
    if (previewIt != previews.end() && previewIt->second.id != 0) {
        ...
        const float aspect = static_cast<float>(tex.height) / static_cast<float>(tex.width);
        ImGui::Image(static_cast<ImTextureID>(tex.id), ImVec2(previewW, previewW * aspect));
    } else {
        const ImVec2 p0 = ImGui::GetCursorScreenPos();
        ImGui::Dummy(ImVec2(previewW, previewW * 9.0f / 16.0f));
        ImGui::GetWindowDrawList()->AddRectFilled(
            p0, ImVec2{p0.x + previewW, p0.y + previewW * 9.0f / 16.0f}, IM_COL32(45, 45, 50, 255),
            4.0f);
        ImGui::GetWindowDrawList()->AddText(ImVec2{p0.x + 8.0f, p0.y + previewW * 9.0f / 32.0f},
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (autoRun) {
            ...
            if (worker.poll(result)) {
                ...
                if (result.ok)
                    uploadNodePreviews(result, previews);
            }
        }
        ...
        if (ImGui::BeginMainMenuBar()) {
            if (autoRun) {
                ...
                if (ImGui::Button("Run")) {
                    autoRun = true;
                    for (auto& n : graphState.nodes)
                        n.running = true;
                    worker.start(services->pipelineExecutor(), buildPipelineGraph(graphState));
        ...
        if (ImGui::BeginPopupModal("Save Graph", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            InputPathWithFolderButton("##savepath", savePathBuf, sizeof(savePathBuf), true);
            ImGui::Spacing();
        ...
        ImGui::SetNextWindowSize(ImVec2(560, 0));
        if (ImGui::BeginPopupModal("Load Graph", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            InputPathWithFolderButton("##loadpath", loadPathBuf, sizeof(loadPathBuf), false);
            ImGui::Spacing();
        ...
        if (showNodes) {
            ...
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.92f, 0.45f, 0.15f, 0.8f));
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.92f, 0.45f, 0.15f, 1.0f));
        ...
        if (showProperties) {
            ...
            ImGui::PopStyleColor(3);
            if (ImGui::IsItemActive())
                rightW = std::clamp(rightW - ImGui::GetIO().MouseDelta.x, 150.0f, 600.0f);
            if (ImGui::IsItemHovered() || ImGui::IsItemActive())
                ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeEW);
            ImGui::SameLine(0.0f, 0.0f);
        ...
        // Throttle to maxFps and compute smoothed FPS
        {
            ...
            const double totalSec = std::chrono::duration<double>(afterSleep - frameStart).count();
            const float instantFps = totalSec > 0.0 ? static_cast<float>(1.0 / totalSec) : 0.0f;
