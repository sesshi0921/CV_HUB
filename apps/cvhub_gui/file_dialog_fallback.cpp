...
std::optional<std::string> windowsDialog(bool saveDialog, const std::string& currentPath) {
    ...
    if (SUCCEEDED(
            CoCreateInstance(clsid, nullptr, CLSCTX_ALL, iid, reinterpret_cast<void**>(&dialog)))) {
        ...
        if (!currentPath.empty())
            dialog->SetFileName(
                widen(std::filesystem::path(currentPath).filename().string()).c_str());
...
std::optional<std::string> linuxDialog(bool saveDialog, const std::string& currentPath) {
    ...
    if (saveDialog && p.extension() != ".json")
        p += ".json";
    return p.string();
}
