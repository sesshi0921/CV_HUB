...
std::optional<std::string> fromUrl(NSURL* url) {
  ...
  if (path == nil) return std::nullopt;
...
void applyNativeAppIcon() {
  @autoreleasepool {
    NSString* bundledIcon =
        [[NSBundle mainBundle] pathForResource:@"app_icon" ofType:@"icns"];
    if (bundledIcon != nil) {
...
std::optional<std::string> openJsonFileDialog(const std::string& currentPath) {
  @autoreleasepool {
    ...
    if (!currentPath.empty()) {
      ...
      if (p.has_parent_path()) [panel setDirectoryURL:[NSURL fileURLWithPath:toNSString(p.parent_path().string())]];
    }
...
std::optional<std::string> saveJsonFileDialog(const std::string& currentPath) {
  @autoreleasepool {
    ...
    std::filesystem::path p = currentPath.empty() ? std::filesystem::path{"graph.json"} : std::filesystem::path{currentPath};
    if (p.has_parent_path()) [panel setDirectoryURL:[NSURL fileURLWithPath:toNSString(p.parent_path().string())]];
    [panel setNameFieldStringValue:toNSString(p.filename().empty() ? std::string{"graph.json"} : p.filename().string())];
    if ([panel runModal] != NSModalResponseOK) return std::nullopt;
